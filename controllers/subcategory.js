  ...
  try {
    ...
    await Category.findOne(
      ...
      async (err, categoryval) => {
        ...
        res
          .status(200)
          .send({ message: "Subcategory saved successfully !!!!!" });
      }
    );
  } catch (err) {
    console.error(err);
...
//get vendors for subcategories
exports.getsubcatvendors = async (req, res) => {
  ...
  if (!subcat) {
    ...
    return res
      .status(400)
      .send("Could not find subcategories for this category");
  }
};
