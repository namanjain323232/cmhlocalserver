...
exports.getvendororders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find({
      ...
      .populate("orderedBy")
      .exec();
    console.log(orders, "bbbbbjjjjjddddsssswwwww");

    // const vendorcal = new VendorCal({
...
exports.listvendorcal = async (req, res) => {
  try {
    ...
    const cal = await VendorCal.find({
      userId: req.params.userid,
...
exports.listvendorcaldate = async (req, res) => {
  try {
    ...
    const cal = await VendorCal.find({
      vendorId: req.params.vendorid,
      availability: {
        $elemMatch: { start: { $gte: startDate, $lte: req.params.end } },
      },
    })
      .populate("vendorInfoId")
      .populate({ path: "availability.timeslots" })
...
exports.currentvendorcal = async (req, res) => {
  console.log(
    "PARAMS from current calendar",
    req.params.start,
    new Date(new Date(req.params.start).setHours(23, 59, 59))
  );
  try {
    const cal = await VendorCal.findOne({
      userId: req.params.userid,
      availability: {
        $elemMatch: {
          start: {
...
exports.updatevendorcal = async (req, res) => {
  try {
    console.log(
      "Value from vendor calendar edit before ",
      req.body.availability[0].timeslots,
      req.params
    );
    const cal = await VendorCal.findOneAndUpdate(
      {
        userId: req.params.userid,
        availability: {
          $elemMatch: {
      ...
      {
        $push: {
          "availability.$.timeslots": req.body.availability[0].timeslots,
        },
      },
      { new: true }
    );
...
exports.createbulkavail = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("Server error for bulk vendor availability", err.message);
  }
};
