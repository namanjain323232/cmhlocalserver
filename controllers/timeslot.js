...
exports.createslot = async (req, res) => {
  ...
  try {
    ...
    if (slot) {
      return res
        .status(400)
        .json({ errors: [{ msg: "Timeslot already exists !!!" }] });
    }
...
exports.listslots = async (req, res) => {
  try {
    timeslots = await Timeslot.find({}).sort("startSlot").exec();
    if (!timeslots) {
...
exports.listBlockedSlots = async (req, res) => {
  try {
    timeslots = await Order.find({
      vendors: { $elemMatch: { vendor: req.params.vendorid } },
      // bookingDate: { $gte: date, $lte: date },
    })
      .select("bookingDate timeslotsSE")
      .exec();
    if (!timeslots) {
      return res.status(400).send("No timeslots were found !!!!");
    }
    console.log(timeslots, "jdbjsbdskjnsldnslk");
    res.json(timeslots);
  } catch (err) {
    console.error(err);
    res.status(500).send("Failed to find timeslots !!!!");
...
exports.editslot = async (req, res) => {
  try {
    ...
    const timeslot = await Timeslot.findOneAndUpdate(
      { _id: req.params._id },
      { startSlot: req.body.startSlot, endSlot: req.body.endSlot }
    ).exec();
    if (!timeslot) {
